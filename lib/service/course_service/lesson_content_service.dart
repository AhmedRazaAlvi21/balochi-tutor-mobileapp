import 'package:balochi_tutor/models/course_model/lesson_content_response_model.dart';
...
class LessonContentService {
  Future<ApiCallResponse<LessonContentResponseModel>> callLessonContentService(BuildContext context, int id) async {
    try {
      var response = await Api().getRequest(context, "${AppUrl.lessonContentApi}/$id/content", sendToken: true);
      debugPrint("lesson content day Response: $response");
      LessonContentResponseModel responseModel = LessonContentResponseModel.fromJson(response);
      debugPrint("lesson content Response Model: ${responseModel.toJson()}");
