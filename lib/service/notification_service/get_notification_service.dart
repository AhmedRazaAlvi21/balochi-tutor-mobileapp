import 'package:balochi_tutor/models/notification_model/get_notifications_response_model.dart';
...
class GetNotificationService {
  Future<ApiCallResponse<GetNotificationResponseModel>> callGetNotificationService(BuildContext context) async {
    try {
      var response = await Api().getRequest(context, AppUrl.notificationsApi, sendToken: true);
      debugPrint("notifications Response: $response");
      GetNotificationResponseModel responseModel = GetNotificationResponseModel.fromJson(response);
      debugPrint("notifications Model: ${responseModel.toJson()}");
